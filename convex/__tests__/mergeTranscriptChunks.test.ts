...
describe('mergeTranscriptChunks', () => {
  ...
  it('renumbers segment ids across chunks', () => {
    // whisper restarts ids at 0 per chunk; transcript rows and ad tagging key
    // off segment identity, so ids must stay unique across the episode
    const merged = mergeTranscriptChunks(
      [
        chunk('first', [[0, 5, 'a'], [5, 10, 'b']], 10),
        chunk('second', [[0, 5, 'c'], [5, 10, 'd']], 10),
      ],
      { chunkByteLengths: [1 * MB, 1 * MB] }
    );

    expect(merged.segments?.map((s) => s.id)).toEqual([0, 1, 2, 3]);
  });

  it('carries speaker labels through from diarized chunks', () => {
    const merged = mergeTranscriptChunks(
      [
        {
          text: 'hello',
          duration: 10,
          segments: [
            { id: 0, start: 0, end: 5, text: 'hello', speaker: 'A' },
            { id: 1, start: 5, end: 10, text: 'hi', speaker: 'B' },
          ],
        },
      ],
      { chunkByteLengths: [1 * MB] }
    );

    expect(merged.segments?.map((s) => s.speaker)).toEqual(['A', 'B']);
  });
});
